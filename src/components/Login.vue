        ...
        methods: {
          login(){
            ...
            if(this.ruleForm.user===''){
              ...
              this.$axios.get("http://127.0.0.1/user/get",{
                params:{user:this.ruleForm.user,pass:this.ruleForm.pass}
              }).then(res=>{
                console.log('res',res);
                if(res.data.status==200){
                  sessionStorage.setItem('ActiveUser',JSON.stringify(res.data.user)) //将当前登录用户信息存入会话存储空间
                  this.$router.replace({
                  name:'index'
                  })
