<template>
    <div>
      ...
      >
        ...
        <el-table-column prop="operate" label="操作" width="300">
          <template slot-scope="scope">
            <el-button style="margin-right:5px" icon="el-icon-edit-outline" @click="view(scope.row)" size="small" type="text">查看</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-dialog
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="saveData">更 新</el-button>
        <el-button type="primary" @click="acceptOrder">确认验收</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
    ...
    methods:{
      get(){
          this.$axios.get('http://127.0.0.1/tasklist/acceptableTaskGet').then(res=>{
      ...
      // 更新
      saveData(){
        this.$axios.get("http://127.0.0.1/tasklist/update",{
      ...
      //  确认验收 
      acceptOrder(){
        this.$axios.get("http://127.0.0.1/tasklist/acceptOrder",{
          params:{任务编号:this.form.任务编号}
        }).then(res=>{
          if(res.data.status==200){
            this.get()
            this.$message.success('操作成功')
