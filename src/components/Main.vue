<template>
  <div>
    <div style="margin:5px">
      ...
      <!-- <el-select v-model="searchInfo.sex" placeholder="请选择性别" style="margin-left:5px">
  ...
  >
        ...
        <el-table-column prop="sex" label="性别" width="60">
          <template slot-scope="scope">
            <el-tag
              :type="scope.row.sex == true ? 'primary' : 'success'"
              disable-transitions>{{(scope.row.sex==true?'男':'女')}}</el-tag>
          </template>
        </el-table-column>
...
export default {
    ...
    methods:{
      ...
      //保存
      saveData(){
        this.$axios.get(this.isEdit?"http://127.0.0.1/list/update":"http://127.0.0.1/list/add",{
      ...
      handleDelete(row){
          this.$confirm("您确信要删除吗？", "提示").then(()=>{
            this.$axios.get("http://127.0.0.1/list/del",{params:{id:row.id}}).then(res=>{
            if(res.data.status==200)
      ...
      getMockList(){
        ...
        this.$axios.get('http://127.0.0.1/list/all').then(res=>{
          this.dataPage.list=res.data
          this.dataPage.total=res.data.length
        }).catch(err=>{
      ...
      postMockList(){
        ...
        // })
        this.$axios.post('http://127.0.0.1/list/page',{pageNum:this.dataPage.pageNum,pageSize:this.dataPage.pageSize}).then(res=>{
          this.dataPage.list=res.data.data.results
          this.dataPage.pageSize=res.data.data.pageSize
          this.dataPage.pageNum=res.data.data.pageNo
        }).catch(err=>{
          console.log('获取数据失败' + err);
        })
      }
